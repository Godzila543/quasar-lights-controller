...
export const useDatabase = defineStore('database', {
  state: () => ({
    ...
    editedGenerator: {
      ...
      particleSharpness: 1,
      persistence: 1,
      colorMethod: 'intensity',
      intensityMethod: 'fade-in-out',
      pulseCount: 1,
      initialPositionMethod: 'set',
      initialPosition: { min: 0, max: 600 },
      initialVelocityMethod: 'set',
      initialVelocity: { min: -1, max: 1 },
      calculatedAttribute: 'position',
      calculationMethod: 'constant',
      calculatedAttributeValue: 0,
      calculatedAttributeStrength: 1,
      calculatedAttributeRange: { min: -1, max: 1 },
    } as Generator,
  ...
  actions: {
    ...
    setActiveGenerator(generator: Generator) {
      ...
      this.bt.sendGenerator(generator);
    },
